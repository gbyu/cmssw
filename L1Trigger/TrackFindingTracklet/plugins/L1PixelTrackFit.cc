...
// Class:      L1PixelTrackFitAlgo
// 
/**\class L1PixelTrackMatchAlgo 

 Description: Producer of a L1PixelTrack, for the algorithm matching a L1Track to the PixHits

 Implementation:
     [Notes on implementation]
*/
//
// Original Author:  G. Yu based on the L1TkElectronProducer & L1PixelTrackFit 
//         Created:  Mon Oct 16 2017
// $Id$
//
//
// -*- C++ -*-
...
//#include "L1Trigger/L1TTrackMatch/interface/L1PixelTrackFit.h"
...
public:
  ...
  explicit L1PixelTrackFit(const edm::ParameterSet&);
  ~L1PixelTrackFit();
...
private:
  ...
  edm::InputTag SiPixRecHitInputTag;

  
  const edm::EDGetTokenT< std::vector< TTTrack< Ref_Phase2TrackerDigi_ > > > trackToken;
  const edm::EDGetTokenT< SiPixelRecHitCollection > tokenPixelRecHits_;
};
...
void
L1PixelTrackFit::beginJob()
{
}

// ------------ method called once each job just after ending the event loop  ------------
void
L1PixelTrackFit::endJob() {
}
...
L1PixelTrackFit::L1PixelTrackFit(edm::ParameterSet const& iConfig) : // :   config(iConfig)
  ...
  tokenPixelRecHits_(consumes< SiPixelRecHitCollection > (iConfig.getParameter<edm::InputTag>("SiPixRecHitInputTag") ) )
{
...
void L1PixelTrackFit::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  std::unique_ptr< std::vector< TTPixelTrack > > result( new std::vector< TTPixelTrack > );
  ...
  if( !recHits.isValid() ) {
    edm::LogError("L1PixelTrackFit")
      << "\nWarning: SiPixelRecHits not found in the event. Exit"
      << std::endl;
    return;
  }
  ...
  for ( ; detUnitIt != detUnitItEnd; detUnitIt++ ) {
	  ...
	  if ( gp.perp() < 20.0 && fabs(gp.z()) < 50.0){ // reject outer tracker, keep three disks only
            cl_pos.push_back(gp);
            cl_phi.push_back(gp.phi());
	    //std::cout << "r z : "<<gp.perp()<<" "<<gp.z()<<std::endl;
	    int type=4;
	    if (gp.perp()<13.0) type=3;
	    if (gp.perp()<8.0) type=2;
	    if (gp.perp()<5.0) type=1;
	    if (fabs(gp.z())>22.0) type=-1;
	    if (fabs(gp.z())>30.0) type=-2;
	    if (fabs(gp.z())>37.0) type=-3;
	    //std::cout << "r z type : "<<gp.perp()<<" "<<gp.z()<<" "<<type<<std::endl;
	    cl_type.push_back(type);
          }
      } // close recHits loop
  } // close detUnits loop
  /* Double check the pixel geometry*/
  ...
  iEvent.getByToken(trackToken, L1TrackHandle);
  ...
  if (!L1TrackHandle.isValid() ) {
    edm::LogError("L1PixelTrackFit")
      << "\nWarning: L1TkTrackCollectionType not found in the event. Exit."
      << std::endl;
    return;
  }
  ...
  for (iterL1Track = L1TrackHandle->begin(); iterL1Track != L1TrackHandle->end(); ++iterL1Track) {
    ...
    double z0=iterL1Track->getPOCA(npar).z(); // point of closest approach
    ...
    double t=sinh(eta); // what is t? theta? = 1/(tan theta)
                        // Pz = ET*sinh(eta)
                        // P0 = ET*cosh(eta)
	...
	if (adphi>0.06&&adphi<6.22) continue; // window DeltaPhi = 0.12
	double rhit=cl_pos[i].perp();
	if (fabs(z0+t*rhit-cl_pos[i].z())>1.0) continue; //loose cut, z0 & t are track parameters
  ...
  iEvent.put( std::move(result),  "Level1PixelTracks");
}
