...
class L1PixelTrackNtupleMaker : public edm::EDAnalyzer
{
public:

  // Constructor/destructor
  explicit L1PixelTrackNtupleMaker(const edm::ParameterSet& iConfig);
  virtual ~L1PixelTrackNtupleMaker();
...
private:
  ...
  edm::InputTag TTPixelTrackInputTag;
  ...
  edm::EDGetTokenT< std::vector< TTPixelTrack > > ttPixelTrackToken_;
...
//////////////
// CONSTRUCTOR
L1PixelTrackNtupleMaker::L1PixelTrackNtupleMaker(edm::ParameterSet const& iConfig) : 
  config(iConfig)
{
  ...
  TTPixelTrackInputTag      = iConfig.getParameter<edm::InputTag>("TTPixelTrackInputTag");
  ...
  ttPixelTrackToken_ = consumes< std::vector< TTPixelTrack > >(TTPixelTrackInputTag);
...
/////////////
// DESTRUCTOR
L1PixelTrackNtupleMaker::~L1PixelTrackNtupleMaker()
{
}  

//////////
// END JOB
void L1PixelTrackNtupleMaker::endJob()
{
  // things to be done at the exit of the event Loop
  cerr << "L1PixelTrackNtupleMaker::endJob" << endl;

}

////////////
// BEGIN JOB
void L1PixelTrackNtupleMaker::beginJob()
{

  // things to be done before entering the event Loop
  cerr << "L1PixelTrackNtupleMaker::beginJob" << endl;
...
//////////
// ANALYZE
void L1PixelTrackNtupleMaker::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  iEvent.getByToken(ttPixelTrackToken_, TTPixelTrackHandle);
  ...
  if (SaveAllTracks && DoPixelTrack) {
    ...
    for ( iterL1PixelTrack = TTPixelTrackHandle->begin(); iterL1PixelTrack != TTPixelTrackHandle->end(); iterL1PixelTrack++ ) {
      ...
      const edm::Ref<std::vector<TTTrack<Ref_Phase2TrackerDigi_> >, TTTrack<Ref_Phase2TrackerDigi_> > matched_tttrack = iterL1PixelTrack->getL1Track();
...
DEFINE_FWK_MODULE(L1PixelTrackNtupleMaker);
